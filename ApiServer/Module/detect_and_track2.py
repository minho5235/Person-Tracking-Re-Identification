...
from Module.models.experimental import attempt_load
from Module.utils.datasets import LoadStreams, LoadImages
from Module.utils.general import check_img_size, check_requirements, \
                check_imshow, non_max_suppression, apply_classifier, \
                scale_coords, xyxy2xywh, strip_optimizer, set_logging, \
                increment_path
from Module.utils.plots import plot_one_box
from Module.utils.torch_utils import select_device, load_classifier, \
                time_synchronized, TracedModel
from Module.utils.download_weights import download

#For SORT tracking
import skimage
import math
from Module.sort2 import *
...
def save_face_image(face_id, face_image):
    cv2.imwrite(f'face_{face_id}.jpg', face_image)
...
def calculate_people(loc, people):
    ...
    for i in range(loc+1, len(people)):
        ...
        if distance[0] <= 0.355:
            index.append(i)
            people[loc].person.extend(people[i].person)
            people[loc].faces.extend(people[i].faces)
            people[loc].face_image.extend(people[i].face_image)
            people[loc].frame_count.extend(people[i].frame_count)
...
def exist_people(people):
    global spd
    for person in people:
        start_p = -1000.0
        end_p = -1000.0
        for i in range(0, len(person.frame_count)):
            if person.frame_count[i] > end_p + (spd*5/2):
                if i != 0:
                    frame_list = [math.floor(start_p/spd*2), math.ceil(end_p/spd*2)]
                    if end_p - start_p >= spd*3/2:
            ...
            if i == len(person.frame_count) - 1:
                ...
                if end_p - start_p >= spd*3/2:
                    person.exist.append(frame_list)
    people = [person for person in people if person.exist]
    return people
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if start_once:
                ...
                vid_cap.set(cv2.CAP_PROP_POS_FRAMES, half_frames)
                start_once = False
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for x1,y1,x2,y2,conf,detclass in det.cpu().detach().numpy():
                    ...
                    tracked_object_image = im0[y1:y2, x1:x2]
                    tracked_people.append(tracked_object_image)

                frame_count_all = int(vid_cap.get(cv2.CAP_PROP_FRAME_COUNT))
                # Run SORT
                tracked_dets = sort_tracker.update(im0, tracked_people, frame_count_all, dets_to_sort)
                ...
                tracked_dets = sort_tracker.update(img, im0, 0)
            #........................................................
                ...
                if dataset.mode == 'image':
                    ...
                    if vid_path != save_path:  # new video
                        ...
                        if vid_cap:  # video
                            ...
                            global spd
                            spd = vid_cap.get(cv2.CAP_PROP_FPS)
    ...
    people = sort_tracker.getPeople()
    a = len(people)

    for person in people:
        person_one = []
        for i in range(0, len(person.person)):
            rgb_small_frame = np.ascontiguousarray(person.person[i][:, :, ::-1])
            face_locations = face_recognition.face_locations(rgb_small_frame)
            face_encodings = face_recognition.face_encodings(rgb_small_frame, face_locations)
            if len(face_encodings) != 1:
                continue
            top, right, bottom, left = face_locations[0]
            face_image = person.person[i][top:bottom, left:right]
            height, width, channels = face_image.shape
            """if height * width < 8000:
                continue"""
            person.add_face_encoding(face_encodings[0])
            person.add_face_image(face_image)
            person_one.append(person.person[i])
        person.person = person_one

    people = [person for person in people if person.calculate_average_encoding() is not None]
    people = [person for person in people if len(person.faces) > 1]

    b = len(people)
    people = calculate_people(0, people)
    c = len(people)
    people = exist_people(people)
    d = len(people)
    
    print("tracking2")   
    print("얼굴인식전 people수 : "+str(a))
    print("얼굴인식후 people수 : "+str(b))
    print("얼굽합친후 people수 : "+str(c))
    print("5초 이하 스킵한 people수 : "+str(d))
...
def tracking(vidio):
    ...
    parser.add_argument('--source', type=str, default=vidio, help='source')  # file/folder, 0 for webcam
